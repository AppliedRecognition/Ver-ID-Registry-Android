...
fun HomeView(
    ...
    LaunchedEffect(sessionState) {
        when (val state = sessionState) {
            is FaceSessionState.IdentificationComplete -> {
            ...
            is FaceSessionState.CaptureError -> {
                faceSessionViewModel.clearState()
                error = "Identification failed: ${state.error.localizedMessage}"
            }
            else -> {}
        }
    }

    val isCapturing = sessionState is FaceSessionState.Capturing
...
private fun HomeViewContent(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(text = "Ver-ID registry demo") },
                actions = {
